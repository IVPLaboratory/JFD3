...
def haze_robust_loss(Fh_list, Fdeh_list):
      
       
        assert len(Fh_list) == len(Fdeh_list), 

        total_loss = 0.0
        num_features = len(Fh_list)

        for Fh, Fdeh in zip(Fh_list, Fdeh_list):
           
            GAP_Fh = torch.mean(Fh, dim=(2, 3), keepdim=True)  
            GAP_Fdeh = torch.mean(Fdeh, dim=(2, 3), keepdim=True)

            
            feature_map1_flat = GAP_Fh.view(GAP_Fh.size(0), -1)
            feature_map2_flat = GAP_Fdeh.view(GAP_Fdeh.size(0), -1)

            cosine_similarity = F.cosine_similarity(feature_map1_flat, feature_map2_flat, dim=1, eps=1e-8)
            
           
            total_loss += (1-cosine_similarity)
...
class lossCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        if 'blur_inner_feats' in outputs and 'clear_inner_feats' in outputs:
            deblur_loss = self.deblur_loss(outputs['blur_inner_feats'], outputs['clear_inner_feats'])
            losses['loss_deblur'] = self.weight_dict['loss_deblur']*deblur_loss
        if 'blur_backbone_feats'in outputs and 'clear_backbone_feats'in outputs:
            BR_loss =haze_robust_loss(outputs['blur_backbone_feats'], outputs['clear_backbone_feats'])
            
            losses['loss_BR'] =self.weight_dict['loss_BR']* BR_loss
...
class DeblurLoss(nn.Module):
    """Criterion class for computing deblurring losses during training."""

    def __init__(self, loss_type='l1'):
        """Initialize the DeblurLoss module.
        
        Args:
            loss_type (str): Type of loss to use. Options: 'l1', 'l2', 'ssim', 'ms_ssim'
        """
        super().__init__()
        ...
        if loss_type == 'l1':
            ...
            self.loss_fn = SSIM(data_range=1.0, size_average=True, channel=3)
        elif loss_type == 'ms_ssim':
            from pytorch_msssim import MS_SSIM
            self.loss_fn = MS_SSIM(data_range=1.0, size_average=True, channel=3)
        else:
            raise ValueError(f"Unsupported loss type: {loss_type}")

    def forward(self, pred, target):
        """Compute deblurring loss between predicted and target images.
        
        Args:
            pred (Union[torch.Tensor, List[torch.Tensor]]): Predicted deblurred image(s)
            target (Union[torch.Tensor, List[torch.Tensor]]): Target clear image(s)
        ...
        if isinstance(pred, list) and isinstance(target, list):
            ...
            if len(pred) > 3 and len(target) > 3 and len(target)==len(pred) :
                ...
                for i in range(6):
                    if i < 3:
                        loss = l1_loss_fn(pred[i], target[i])
                    else:
                        
                        loss = 1.0 - ssim_loss_fn(pred[i], target[i])
                    total_loss += loss
                return total_loss / 6
           
        else:
            if self.loss_type in ['ssim', 'ms_ssim']:
